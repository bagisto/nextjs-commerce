...
export function hasAuthFlag(): boolean {
  try {
    return localStorage.getItem(COMPARE_AUTH_KEY) === "true";
  } catch {
    return false;
  }
}
...
function persistAuthenticated(): void {
  try {
    localStorage.setItem(COMPARE_STORAGE_KEY, JSON.stringify([...getOrInitCache()]));
    localStorage.setItem(COMPARE_TIMESTAMP_KEY, String(Date.now()));
    localStorage.setItem(COMPARE_AUTH_KEY, "true");
  } catch { }
}
...
export function isCacheStale(ttlMs: number = COMPARE_DEFAULT_TTL_MS): boolean {
  if (!hasAuthFlag()) return true;
  try {
    const ts = localStorage.getItem(COMPARE_TIMESTAMP_KEY);
    if (!ts) return true;
...
export function getLastUpdateTime(): number {
  try {
    return Number(localStorage.getItem(COMPARE_TIMESTAMP_KEY)) || 0;
  } catch {
    return 0;
  }
}
...
export function clearCache(): void {
  ...
  try {
    ...
    localStorage.removeItem(COMPARE_TIMESTAMP_KEY);
    localStorage.removeItem(COMPARE_AUTH_KEY);
  } catch { }
}
...
export function ensureIdsLoaded(
  ...
  ttlMs: number = COMPARE_DEFAULT_TTL_MS
): Promise<void> {
